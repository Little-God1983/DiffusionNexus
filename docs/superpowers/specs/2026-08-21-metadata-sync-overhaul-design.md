...
- [x] **WP1 — Schema + sync state + derivation** · `ModelSyncState` entity/table, `ModelImage` columns, EF migration, pre-migration VACUUM INTO backup (S2), `SyncStateDeriver` (S3) with table-driven tests, downgrade test (S6).
- [x] **WP2 — `LibrarySyncService`** · plan/execute, steps 0–3, retry windows, single-flight guard, logging; `LoraViewerViewModel` wired to it; tile-based phases + per-tile copy deleted; tests with in-memory SQLite per test conventions.
