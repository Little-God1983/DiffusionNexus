...
| New pixels | **Transparent**, shown as a checkerboard with a green tint while previewing. After Apply the editor paints a transparency checkerboard (`TransparencyCheckerboard`, 8 px cells, #666/#999) under the image in `RenderWithZoom`, so the new strips stay visible instead of vanishing into the dark canvas (smoke round 3, 2026-09-03; this also makes removed backgrounds visible). Colouring is the existing Fill tool's job; no colour picker in this panel. |
