...
| New pixels | **Transparent**, shown as a checkerboard with a green tint while previewing. The preview strips and the post-Apply rendering use the **same** checkerboard: `TransparencyCheckerboard` (8 px cells, #666/#999, anchored to the rect so it pans with the image) is painted under the image in `RenderWithZoom` and under each preview strip by `CanvasExtendTool.DrawNewArea`, so the new strips stay visible after Apply instead of vanishing into the dark canvas (smoke round 3, 2026-09-03; this also makes removed backgrounds visible) and the look does not change the moment Apply lands. Colouring is the existing Fill tool's job; no colour picker in this panel. |
| Typed width/height | Two numeric fields (spinner buttons hidden: with them the ~280 px panel showed only three digits). A typed size grows the canvas away from the **image placement** (below). Values below the image size clamp to the image size, show the shrink hint, **and push the clamped size to the tool** so an already-extended axis snaps back with the field (review of PR #555: the first cut left the frame at the old size and the panel contradicted the canvas). |
| Image placement | A 3×3 anchor grid ("Image position") plus **drag the frame**: pressing anywhere inside the green frame (image or new area, off every handle) while there is any extension drags the frame around the image, trading extension between opposite edges; the size never changes. The frame follows the pointer and the image stays put on screen for the whole drag (`PinnedImageRect`: the renderer skips the fit re-centre while `IsMovingFrame`); on release fit mode re-centres the frame. First cut moved the image instead and let fit mode re-centre live, which read as "inverted" (smoke round 4, 2026-09-03). Default anchor is **top-left** (canvas grows right and down), chosen by the user on 2026-09-03 over the centred first cut. A drag sets `CanvasAnchor.Custom`: the grid shows no cell, later typed sizes keep the image's offset from the top-left and grow/shrink at the right and bottom edges. Picking a cell redistributes the current extension at once. `Reset` reverts Custom to the default but keeps a chosen cell. Outpaint keeps its centred default and gets no frame drag (`AllowsFrameMove` is opt-in). |
...
| Apply | Grows layers **and** the working bitmap exactly the way `Crop()` handles both, resets the tool, closes the panel. No undo; Reset recovers. All-or-nothing: the working bitmap is allocated first, `LayerStack.ResizeCanvas` allocates every layer's replacement before swapping any of them in, and a failure anywhere leaves the document untouched (review of PR #555). |
