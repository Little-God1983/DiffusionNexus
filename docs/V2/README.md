...
| [Captioning.md](Captioning.md) | Local AI image captioning with LlamaSharp and CUDA |
...
? (Image Caching,   ?  (EF Core +      ? (API Client)             ?
?  Secure Storage)  ?   SQLite)        ?                          ?
???????????????????????????????????????????????????????????????????
?                     DiffusionNexus.Captioning                     ?
?              (LlamaSharp VLM + CUDA 12 GPU Inference)             ?
???????????????????????????????????????????????????????????????????
...
| `DiffusionNexus.Captioning` | Local AI captioning with LlamaSharp and CUDA 12 |
