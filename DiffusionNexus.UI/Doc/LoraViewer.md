...
- The name is therefore the sole evidence only for a `.pth` / `.ckpt` pickle, which has no header for anything to read. There the markers that can fire (`4x-`, `esrgan`, `ultrasharp`, `lsdir`) do not occur in LoRA names — and the legacy backfill is scoped to exactly that cohort for the same reason.
