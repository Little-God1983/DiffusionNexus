...
The main `LoraDatasetHelperView` follows the **Shell pattern** - it's a minimal container that:
- Hosts the `TabControl` with child tab views
- Injects `DialogService` into the ViewModel hierarchy
- Contains no business logic or UI event handling

```
???????????????????????????????????????????????????????????????????
?              LoraDatasetHelperView (Shell)                       ?
?  - ~55 lines of code                                             ?
?  - TabControl hosting child views                                ?
?  - DialogService injection only                                  ?
???????????????????????????????????????????????????????????????????
?         ?                              ?                         ?
?         ?                              ?                         ?
?  ????????????????????      ???????????????????????              ?
?  ?DatasetManagement ?      ? ImageEditView       ?              ?
?  ?   View           ?      ?                     ?              ?
?  ?  - Drag & drop   ?      ?  - Editor wiring    ?              ?
?  ?  - Selection     ?      ?  - Zoom slider      ?              ?
?  ?  - Keyboard      ?      ?  - Canvas events    ?              ?
?  ????????????????????      ???????????????????????              ?
???????????????????????????????????????????????????????????????????
```
...
Each tab has its own View with encapsulated responsibilities:

#### DatasetManagementView
**Location:** `Views/Tabs/DatasetManagementView.axaml(.cs)`

Handles all Dataset Management tab logic:
- Drag & drop file handling (ZIP extraction, folder scanning)
- Keyboard shortcuts (Ctrl+A, Escape)
- Pointer events for Shift+Click/Ctrl+Click selection
- Description auto-save on TextBox blur

#### ImageEditView
**Location:** `Views/Tabs/ImageEditView.axaml(.cs)`

Handles all Image Editor tab logic:
- `ImageEditorControl` event wiring (zoom, crop, save)
- Zoom slider synchronization
- DialogService for save confirmation dialogs

### Benefits of View Separation

1. **Single Responsibility** - Each view handles only its tab's concerns
2. **Maintainability** - Changes to one tab don't affect others
3. **Testability** - Tab views can be tested in isolation
4. **Reusability** - Tab views could be used in other contexts
5. **Reduced Complexity** - Main shell is ~55 lines vs 400+ lines
...
|-----------|------|
| **Utilities** | |
| MediaFileExtensions | `UI-V2/Utilities/MediaFileExtensions.cs` |
| **Services** | |
...
| **Views** | |
| LoraDatasetHelperView (Shell) | `UI-V2/Views/LoraDatasetHelperView.axaml(.cs)` |
| DatasetManagementView | `UI-V2/Views/Tabs/DatasetManagementView.axaml(.cs)` |
| ImageEditView | `UI-V2/Views/Tabs/ImageEditView.axaml(.cs)` |
| **DI Registration** | |
| App | `UI-V2/App.axaml.cs` |
